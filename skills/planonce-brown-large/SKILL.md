...
7. **Human gate — plan approval.**
8. Execute phase by phase using **fresh context** workers where supported; use provider fallback sequential compact handoffs otherwise.
...
11. Run comprehensive regression/integration/migration/build/lint/types/performance/operational checks and user-facing acceptance validation.
...
13. Audit every requirement/non-goal against the final diff; record residual risk and fresh evidence in `VERIFY.md`.
14. Run `planonce-review` as the mandatory final production-readiness review; separate introduced blockers from pre-existing backlog and require `READY` or explicitly human-accepted `READY_WITH_BACKLOG`.
15. **Human gate — ship.**
